...
- 元素定位与交互：Appium 使用 find_element 方法结合 By 类来定位元素，但由于平台区别，支持的定位方式存在一定不同。具体来说，安卓系统下的 Appium 可通过以下方式进行元素定位：
  ...
  - 长按：通过 TouchAction().longPress() 实现长按操作。
- 手势操作：安卓设备支持多种手势操作，包括
  - 滑动：使用 driver.swipe() 实现从屏幕一端滑动到另一端的操作。
  - 拖拽：使用 TouchAction().press().moveTo().release() 实现元素的拖拽操作。
  - 捏合（缩放）：通过 MultiTouchAction 可以实现缩放操作（双指捏合或扩展）。
...
1. UI 元素交互测试
   ...
   3. 编辑/删除列表：长按列表以检查是否能正确触发编辑或删除选项。测试移动列表功能是否正常。
...
本节中，我们考虑对该应用进行性能测试。通过向应用中添加大量待办事项的方式，测试应用在大量数据下的性能表现。在极限条件下（如大量数据输入或高并发用户）测试应用的稳定性和性能的方式被称为压力测试。
